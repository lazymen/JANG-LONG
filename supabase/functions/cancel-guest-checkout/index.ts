...
function readToken(body: JsonObject, key: string): string {
    ...
    if (typeof rawValue !== "string") {
        throw new CheckoutError(
            ...
            "예약 정보를 확인하지 못했습니다. 장바구니에서 다시 시도해주세요.",
        );
    }
...
function convertDatabaseError(message: string): CheckoutError {
    ...
    if (message.includes("CHECKOUT_SESSION_CLOSED")) {
        return new CheckoutError(
            ...
            "이미 종료된 예약입니다. 장바구니에서 다시 시도해주세요.",
        );
    }

    if (message.includes("CHECKOUT_PAYMENT_STATUS_UNRESOLVED")) {
        return new CheckoutError(
            409,
            "CHECKOUT_PAYMENT_STATUS_UNRESOLVED",
            "결제 상태를 확인 중인 예약은 취소할 수 없습니다. 잠시 후 다시 시도해주세요.",
        );
    }
    ...
    fetch: withSupabase(
        ...
        async (request, context) => {
            try {
                ...
                const supabaseAdmin = context.supabaseAdmin as SupabaseClient<
                    Database
                >;
                ...
                if (error) {
                    console.error("cancel_guest_checkout failed", {
                ...
                return jsonResponse({ ok: true, reservation });
            } catch (error) {
                ...
                console.error("Unexpected checkout cancellation error");
