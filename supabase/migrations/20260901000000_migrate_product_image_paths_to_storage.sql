...
alter table public.products
    drop constraint products_main_image_first;
...
begin
    if exists (
        ...
        cross join lateral unnest(
            coalesce(products.images, array[]::text[])
        ) as image_rows(image_path)
        where
            image_rows.image_path !~ '^[A-Za-z0-9][A-Za-z0-9._-]*$'
            and image_rows.image_path !~ '^[0-9]{4}/[A-Za-z0-9][A-Za-z0-9._-]*$'
            and image_rows.image_path !~ '^images/products/[0-9]{4}/[A-Za-z0-9][A-Za-z0-9._-]*$'
    ) then
        raise exception
            'PRODUCT_IMAGE_PATH_FORMAT_UNSUPPORTED';
    end if;
    ...
    if exists (
        ...
        where
            (
                image_rows.image_path ~ '^[0-9]{4}/'
                and split_part(image_rows.image_path, '/', 1)
                    <> products.id
            )
            or (
                image_rows.image_path ~ '^images/products/[0-9]{4}/'
                and split_part(image_rows.image_path, '/', 3)
                    <> products.id
            )
    ) then
        raise exception
            'PRODUCT_IMAGE_PATH_PRODUCT_ID_MISMATCH';
    end if;
end;
$$;
...
set images = coalesce(
    (
        select array_agg(
            case
                when image_rows.image_path ~ '^[0-9]{4}/' then
                    image_rows.image_path

                when image_rows.image_path
                    ~ '^images/products/[0-9]{4}/' then
                    regexp_replace(
                        image_rows.image_path,
                        '^images/products/([0-9]{4}/[A-Za-z0-9][A-Za-z0-9._-]*)$',
                        '\1'
                    )

                else
                    products.id || '/' || image_rows.image_path
            end
            order by image_rows.position
        )
        from unnest(products.images)
            with ordinality as image_rows(image_path, position)
    ),
    array[]::text[]
)
where products.images is not null;

alter table public.products
    add constraint products_main_image_first
    check (
        images is null
        or cardinality(images) = 0
        or images[1] ~ '^[0-9]{4}/main\.jpg$'
    );
...
begin
    ...
    (
        ...
        from unnest(products.images)
            with ordinality as product_images(
                image_path,
                position
            )
