-- JANG LONG
-- Connect the private shipping calculator to atomic guest checkout.
--
-- The trusted backend now supplies only a remote-area classification.
-- It can no longer supply or choose the shipping amount itself. The database
-- calculates the amount from the authoritative product subtotal and the
-- single private checkout_shipping_settings row.
--
-- Remote-area classification is still a later backend responsibility. This
-- function must not be connected to the storefront until that classifier and
-- the Supabase Edge Function are implemented.

begin;

-- Preserve why the shipping amount was charged. The actual amount already
-- remains frozen in orders.shipping_fee for historical accuracy.
alter table public.orders
    add column is_remote_area boolean not null default false;

comment on column public.orders.is_remote_area is
'Trusted backend remote-area classification captured when checkout starts.';


-- Remove the former entry point that accepted an arbitrary shipping amount.
-- The replacement accepts only the trusted remote-area classification.
drop function public.start_guest_checkout(
    text,
    text,
    text,
    text,
...
create function public.start_guest_checkout(
...
begin
    ...
    -- Retrying the same checkout returns the original order instead of
    -- creating another order or extending the reservation.
    ...
    if found then
        ...
        if v_existing_order.recovery_token_hash <> v_recovery_token_hash
           ...
           or v_existing_order.is_remote_area <> p_is_remote_area
    ...
    -- The database, not the browser or Edge Function, decides the amount.
    v_shipping_fee := public.calculate_checkout_shipping_fee(
        v_subtotal,
        p_is_remote_area
    );
...
'Private atomic guest checkout with server-calculated shipping. Call from a trusted backend only.';
